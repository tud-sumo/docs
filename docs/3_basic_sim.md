...
By default, a progress bar is automatically created when simulating more than 10 steps at a time. This can be skipped by setting `suppress_pbar = False`. An `include_insertion_delay` is also included that denotes whether to include delay from vehicles waiting to enter the simulation (likely due to their insertion location being blocked) into delay calculations. These vehicles are assumed to experience delay for the full time step, which is then added to the total for this time step. This can dramatically increase network-wide delay if the demand profile is not designed correctly.
